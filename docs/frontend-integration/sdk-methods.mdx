...
:::tip
Use `transactionHash` for generating links to solana explorers. For example to https://explorer.solana.com
:::
...
```ts
// With txStage
const transactionHash = await solidoSDK.unStake({
    amount: 20,
    wallet: walletInstance,
    setTxStage: setTxStageCallback,
});
...
```ts
const { transaction: unStakeTransaction} = await solidoSDK.getUnStakeTransaction({
    amount: 20,
    payerAddress: new PublicKey(wallet.publicKey),
});
```
...
_type:_ `Promise<{transaction: Transaction, stakeAccountAddress: Publickey}>`
...
```ts
const { apy, totalStaked, stakers, marketCap } = await solidoSDK.getLidoStatistics();
